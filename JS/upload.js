...
const Drop_Area = document.getElementsByClassName('PageContainer')[0];
...
Upload_Listeners = async () => {
  ...
  ['dragenter', "dragover"].forEach(event => {
    ...
    function highlight(e) { if (e.dataTransfer.types[0] == "Files") {Drop_Area.classList.add('highlight')} }
  });
  ['dragleave', 'drop'].forEach(event => {
    Drop_Area.addEventListener(event, unhighlight, false)
    function unhighlight(e) {Drop_Area.classList.remove('highlight')}
  });
